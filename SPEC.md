...
- 문제/목표: 투자자가 구독한 종목의 하루치 언론·미디어·SNS 텍스트를 자동으로 수집·분석해 핵심 요약과 인사이트 리포트를 Slack으로 전달한다. 데이터 수집부터 분석, 배포까지 전 과정을 신뢰성 있게 자동화하는 것이 목표다.
- 핵심 사용 시나리오:
  1. 사용자가 Slack Slash Command 또는 모달에서 관심 종목을 구독·해지하거나 알림 빈도를 조정한다.
  2. 스케줄러가 1일 주기로 각 종목 관련 텍스트를 수집 API/크롤러를 통해 가져와 정제 큐에 적재한다.
  3. Python 분석 파이프라인이 텍스트를 정제하고 감성·토픽·이상 징후를 계산한 뒤 요약 리포트를 생성한다.
...
- 엔티티/필드:
  ...
  - ProcessedInsight: insight_id, ticker, summary_text, sentiment_score, keywords[], anomaly_score, source_refs[], generated_at
...
- 저장소/연동/비고:
  ...
  - 외부 API: 공시/뉴스 제공자(DART, News API 등), SNS 공식 API 혹은 합법적 RSS
...
- 분석 단계:
  - Python 기반 NLP 파이프라인: 텍스트 정제 → 언어별 모델 선택 → 감성/토픽 분석 → 요약 생성(외부 LLM + 경량 ML 혼합)
  - 모델 전략 대안
    1. 자체 Fine-tuned 모델: 데이터 제어, 운영 복잡·리소스 부담
    2. 외부 LLM API: 빠른 구현, 비용·데이터 의존성
  - 결론: 초기엔 외부 LLM(API Guardrail) + 경량 ML 모델 병행, 장기적으로 비용·규제 평가 후 자체 모델 전환 고려
- 봇 시스템 단계:
...
- 성능/보안/접근성:
  ...
  - 보안: OAuth/토큰 KMS 보관, 네트워크 아웃바운드 제한, 최소 권한 IAM
...
- 로깅/에러 처리/관찰성:
  ...
  - 메트릭: 파이프라인 단계별 처리량, 실패율, 지연시간(CloudWatch/Prometheus)
  - 알림: 실패율 임계치, Slack 전송 실패, LLM 비용 급증 시 PagerDuty/Slack Alert
...
- repo 구조:
  ...
  - `/analysis` (Python): NLP 파이프라인, 프롬프트 템플릿, 모델 관리
...
- `.env.example`: Slack 토큰, 외부 API 키, DB/Redis 접속 정보 placeholder 및 설명
