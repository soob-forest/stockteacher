...
## 착수 TODO (Chroma 폴백/안정화 스프린트)
- [ ] `api/vector_search.py`에 Chroma heartbeat/타임아웃 처리 추가 후 실패 시 키워드·티커 기반 폴백 경로 반환 (실 서비스 구현에서는 fake 클라이언트 사용 금지).
- [ ] `api/chat_service.py` RAG 경로에 동일한 헬스체크/타임아웃/폴백 적용, 실패 시 컨텍스트 없이 스트림 지속.
- [ ] 폴백 발생 시 공통 로그 필드(`trace_id`, `fallback_reason`, `duration_ms`) 추가 및 메트릭 훅 정리.
- [ ] `ingestion/tasks/embed.py` 실행/삭제 흐름에 Chroma 장애 대응 로그 보강(임베딩 누락/삭제 실패 알림).
- [ ] 테스트: Chroma 응답 실패 → 폴백 반환/스트림 지속 시나리오 추가(실제 클라이언트 모킹 없이 httpx transport mock만 사용).
